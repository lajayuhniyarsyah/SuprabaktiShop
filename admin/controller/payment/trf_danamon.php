...
class ControllerPaymentTrfDanamon extends Controller {
	...
	public function index() {
		...
		$this->data['text_logo'] = $this->language->get('text_trf_danamon');
		...
		$this->data['action'] = $this->url->link('payment/trf_danamon', 'token=' . $this->session->data['token'], 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/payment', 'token=' . $this->session->data['token'], 'SSL');
		
		if (isset($this->request->post['trf_danamon_accountno'])) {
			$this->data['trf_danamon_accountno'] = $this->request->post['trf_danamon_accountno'];
		} else {
			$this->data['trf_danamon_accountno'] = $this->config->get('trf_danamon_accountno');
		}
		
		if (isset($this->request->post['trf_danamon_accountname'])) {
			$this->data['trf_danamon_accountname'] = $this->request->post['trf_danamon_accountname'];
		} else {
			$this->data['trf_danamon_accountname'] = $this->config->get('trf_danamon_accountname');
		}
		
		if (isset($this->request->post['trf_danamon_total'])) {
			$this->data['trf_danamon_total'] = $this->request->post['trf_danamon_total'];
		} else {
			$this->data['trf_danamon_total'] = $this->config->get('trf_danamon_total');
		} 
				
		if (isset($this->request->post['trf_danamon_order_status_id'])) {
			$this->data['trf_danamon_order_status_id'] = $this->request->post['trf_danamon_order_status_id'];
		} else {
			$this->data['trf_danamon_order_status_id'] = $this->config->get('trf_danamon_order_status_id');
		} 
		...
		if (isset($this->request->post['trf_danamon_geo_zone_id'])) {
			$this->data['trf_danamon_geo_zone_id'] = $this->request->post['trf_danamon_geo_zone_id'];
		} else {
			$this->data['trf_danamon_geo_zone_id'] = $this->config->get('trf_danamon_geo_zone_id');
		} 
		...
		if (isset($this->request->post['trf_danamon_status'])) {
			$this->data['trf_danamon_status'] = $this->request->post['trf_danamon_status'];
		} else {
			$this->data['trf_danamon_status'] = $this->config->get('trf_danamon_status');
		}
		
		if (isset($this->request->post['trf_danamon_sort_order'])) {
			$this->data['trf_danamon_sort_order'] = $this->request->post['trf_danamon_sort_order'];
		} else {
			$this->data['trf_danamon_sort_order'] = $this->config->get('trf_danamon_sort_order');
		}

		$this->template = 'payment/trf_danamon.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function validate() {
		...
		if (!$this->request->post['trf_danamon_accountno']) {
			$this->error['accountno'] = $this->language->get('error_accountno');
		}

		if (!$this->request->post['trf_danamon_accountname']) {
