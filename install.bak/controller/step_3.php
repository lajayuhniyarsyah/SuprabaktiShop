...
class ControllerStep3 extends Controller {
	...
	private function validate() {
		if (!$this->request->post['db_host']) {
			$this->error['db_host'] = 'Host harap diisi!';
		}

		if (!$this->request->post['db_user']) {
			$this->error['db_user'] = 'User harap diisi!';
		}

		if (!$this->request->post['db_name']) {
			$this->error['db_name'] = 'Nama Database harap diisi!';
		}
		
		if (!$this->request->post['username']) {
			$this->error['username'] = 'Username harap diisi!';
		}

		if (!$this->request->post['password']) {
			$this->error['password'] = 'Password harap diisi!';
		}

		if ((utf8_strlen($this->request->post['email']) > 96) || !preg_match('/^[^\@]+@.*\.[a-z]{2,6}$/i', $this->request->post['email'])) {
			$this->error['email'] = 'E-Mail harap diisi dengan benar!';
		}

		if (!$connection = @mysql_connect($this->request->post['db_host'], $this->request->post['db_user'], $this->request->post['db_password'])) {
			$this->error['warning'] = 'Error: Tidak bisa melakukan koneksi ke database, harap isi semua isian dengan benar!';
		} else {
			if (!@mysql_select_db($this->request->post['db_name'], $connection)) {
				$this->error['warning'] = 'Error: Database tidak ditemukan!';
			}
		...
		if (!is_writable(DIR_OPENCART . 'config.php')) {
			$this->error['warning'] = 'Error: Tidak bisa menulisi config.php. Tolong set dulu permission-nya di: ' . DIR_OPENCART . 'config.php!';
		}
	
		if (!is_writable(DIR_OPENCART . 'admin/config.php')) {
			$this->error['warning'] = 'Error: Tidak bisa menulisi config.php. Tolong set dulu permission-nya di: ' . DIR_OPENCART . 'admin/config.php!';
		}	
