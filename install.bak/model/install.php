...
class ModelInstall extends Model {
	public function mysql($data) {
		...
		$file = DIR_APPLICATION . 'opencart1.sql';
			...
			foreach($sql as $line) {
				...
				if (($sql != '') && (substr($tsl, 0, 2) != "--") && (substr($tsl, 0, 1) != '#')) {
					...
					if (preg_match('/;\s*$/', $line)) {
						...
						$query = str_replace("CREATE TABLE IF NOT EXISTS `oc_", "CREATE TABLE IF NOT EXISTS `" . $data['db_prefix'], $query);
						$query = str_replace("INSERT INTO `oc_", "INSERT INTO `" . $data['db_prefix'], $query);
						$query = str_replace("DROP VIEW IF EXISTS `oc_", "DROP VIEW IF EXISTS `" . $data['db_prefix'], $query);
						$query = str_replace("CREATE VIEW `oc_", "CREATE VIEW `" . $data['db_prefix'], $query);
						$query = str_replace("FROM ((`oc_", "FROM ((`" . $data['db_prefix'], $query);
						$query = str_replace("LEFT JOIN `oc_", "LEFT JOIN `" . $data['db_prefix'], $query);
