...
class ModelUpgrade extends Model {
			...
			foreach($sql as $line) {
				...
				if (($sql != '') && $tsl && (substr($tsl, 0, 2) != "--") && (substr($tsl, 0, 1) != '#')) {
					...
					if (preg_match('/;\s*$/', $line)) {
						...
						$query = str_replace("DROP VIEW IF EXISTS `oc_", "DROP VIEW IF EXISTS `" . DB_PREFIX, $query);
						$query = str_replace("CREATE TABLE `oc_", "CREATE TABLE `" . DB_PREFIX, $query);
						$query = str_replace("CREATE VIEW `oc_", "CREATE VIEW `" . DB_PREFIX, $query);
						$query = str_replace("INSERT INTO `oc_", "INSERT INTO `" . DB_PREFIX, $query);
						$query = str_replace("DELETE FROM `oc_", "DELETE FROM `" . DB_PREFIX, $query);
    ...
    public function update_language($db, $table) {
		$column_query = $db->query("SELECT * FROM " . DB_PREFIX . $table . " WHERE language_id = 3");
		if (!$column_query->num_rows) {
           $db->query("CREATE TEMPORARY TABLE `tmp` SELECT * FROM `" . DB_PREFIX . $table . "` WHERE language_id = 2");
           $db->query("UPDATE `tmp` SET language_id = 3");
           $db->query("INSERT INTO `" . DB_PREFIX . $table . "` SELECT * FROM `tmp`");
		   $db->query("DROP TEMPORARY TABLE IF EXISTS `tmp`");
		}
    }

	public function modifications($data) {
		...
		// Testimonial 'title'  and 'description' field moved to new testiomonial table. Need to loop through and move over.
		$column_query = $db->query("DESC " . DB_PREFIX . "testimonial `title`");
		if ($column_query->num_rows) {
			$testimonial_query = $db->query("SELECT * FROM " . DB_PREFIX . "testimonial");
			foreach ($testimonial_query->rows as $testimonial) {
			    if ($testimonial['description'] == '') {
			        $desc_query = $db->query("SELECT * FROM " . DB_PREFIX . "testimonial_description WHERE testimonial_id = '" . (int)$testimonial['testimonial_id'] . "' AND language_id = '2'");
			        if ($desc_query->num_rows) {
				        $desc = $desc_query->rows[0];
                        $db->query("UPDATE " . DB_PREFIX . "testimonial SET title = '" . $desc['title'] . "', description = '" . $desc['description'] . "' WHERE testimonial_id = '" . (int)$testimonial['testimonial_id'] . "'");
			        }
                }
            }
		}

        $this->update_language($db, "attribute_description");
        $this->update_language($db, "attribute_group_description");
        $this->update_language($db, "banner_image_description");
        $this->update_language($db, "category_description");
        $this->update_language($db, "customer_group_description");
        $this->update_language($db, "download_description");
        $this->update_language($db, "information_description");
        $this->update_language($db, "length_class_description");
        $this->update_language($db, "option_description");
        $this->update_language($db, "option_value_description");
        $this->update_language($db, "order_status");
        $this->update_language($db, "product_attribute");
        $this->update_language($db, "product_description");
        $this->update_language($db, "return_action");
        $this->update_language($db, "return_reason");
        $this->update_language($db, "return_status");
        $this->update_language($db, "stock_status");
        $this->update_language($db, "voucher_theme_description");
        $this->update_language($db, "weight_class_description");
		...
		if ($sql = file($file)) {
			...
			foreach($sql as $line) {
				...
				if (($sql != '') && (substr($tsl, 0, 2) != "--") && (substr($tsl, 0, 1) != '#')) {
					...
					if (preg_match('/;\s*$/', $line)) {
						$query = str_replace("TRUNCATE `oc_", "TRUNCATE `" . DB_PREFIX, $query);
						$query = str_replace("INSERT INTO `oc_", "INSERT INTO `" . DB_PREFIX, $query);
