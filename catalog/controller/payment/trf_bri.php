...
class ControllerPaymentTrfBri extends Controller {
	protected function index() {
		...
		$this->data['accountno'] = $this->config->get('trf_bri_accountno');
		$this->data['accountname'] = $this->config->get('trf_bri_accountname');

		$this->data['continue'] = $this->url->link('checkout/success');
		
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/trf_bri.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/trf_bri.tpl';
		} else {
			$this->template = 'default/template/payment/trf_bri.tpl';
		}	
	...
	public function confirm() {
		...
		$comment .= "<p><b>" . $this->language->get('text_accountno') . "</b>" . $this->config->get('trf_bri_accountno') . "<br/>";
		$comment .= "<b>" . $this->language->get('text_accountname') . "</b>" . $this->config->get('trf_bri_accountname') . "</p>";
		$comment .= "<p>" . $this->language->get('text_detail2') . "</p>";
		
		$this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('trf_bri_order_status_id'), $comment, true);
	}
}
?>
